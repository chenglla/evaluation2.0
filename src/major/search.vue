<template>
  <div class="encyList">
    ...
    <div class="search">
      ...
      <input  v-model="key" type="text" @change="getMajor(key)" placeholder='搜索关键词'>
      <i class="iconfont iconsousuo"></i>
    </div>
    <div class="ency_split"></div>
    <div class="ency-middle">
      <div style="padding:20px 0 5px 0;width: 100%">
        <div  class="img" v-if ="majorList.length == 0">
        ...
        <div v-else>
          <div class="major-list" v-for="(sub,index) in majorList"  :key="index" >
            <el-button type="text" @click.stop="gotoPage1(sub.education,sub.majorName)">
              {{sub.majorName}}
            </el-button>
          </div>
        </div>
...
<script>
import {getMajorInfoLike} from '@/api/index'
export default {
  data() {
    return {
      key:'',
